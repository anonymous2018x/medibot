		...
		$pcConfig = array(
			...
			'symptoms' => array(   
				'infection' => array(   
					...
					'forced-where' => '',
					'display-fields' => array(1 => 'Name', 2 => 'Infection'),
					'display-field-names' => array(1 => 'name', 2 => 'infection'),
					'sortable-fields' => array(0 => '`symptoms`.`id`', 1 => 2, 2 => '`infections1`.`name`'),
					'records-per-page' => 10,
					...
					'template' => 'children-symptoms',
					'template-printable' => 'children-symptoms-printable',
					'query' => "SELECT `symptoms`.`id` as 'id', `symptoms`.`name` as 'name', IF(    CHAR_LENGTH(`infections1`.`name`), CONCAT_WS('',   `infections1`.`name`), '') as 'infection' FROM `symptoms` LEFT JOIN `infections` as infections1 ON `infections1`.`id`=`symptoms`.`infection` "
				)
			),
			'descriptions' => array(   
			)
