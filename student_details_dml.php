...
function student_details_insert(){
	...
	sql('insert into `student_details` set       `full_name`=' . (($data['full_name'] !== '' && $data['full_name'] !== NULL) ? "'{$data['full_name']}'" : 'NULL') . ', `infection`=' . (($data['infection'] !== '' && $data['infection'] !== NULL) ? "'{$data['infection']}'" : 'NULL') . ', `symptom`=' . (($data['symptom'] !== '' && $data['symptom'] !== NULL) ? "'{$data['symptom']}'" : 'NULL') . ', `category`=' . (($data['category'] !== '' && $data['category'] !== NULL) ? "'{$data['category']}'" : 'NULL') . ', `reg_no`=' . (($data['reg_no'] !== '' && $data['reg_no'] !== NULL) ? "'{$data['reg_no']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
...
function student_details_update($selected_id){
	...
	$o=array('silentErrors' => true);
	sql('update `student_details` set       `full_name`=' . (($data['full_name'] !== '' && $data['full_name'] !== NULL) ? "'{$data['full_name']}'" : 'NULL') . ', `infection`=' . (($data['infection'] !== '' && $data['infection'] !== NULL) ? "'{$data['infection']}'" : 'NULL') . ', `symptom`=' . (($data['symptom'] !== '' && $data['symptom'] !== NULL) ? "'{$data['symptom']}'" : 'NULL') . ', `category`=' . (($data['category'] !== '' && $data['category'] !== NULL) ? "'{$data['category']}'" : 'NULL') . ', `reg_no`=' . (($data['reg_no'] !== '' && $data['reg_no'] !== NULL) ? "'{$data['reg_no']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
...
function student_details_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if(is_file(dirname(__FILE__).'/hooks/student_details.category.csv')){
		$category_data = addslashes(implode('', @file(dirname(__FILE__).'/hooks/student_details.category.csv')));
			...
			$j("#infection-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_infection__RAND__.value, t: 'student_details', f: 'infection' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'student_details', f: 'infection' }; },
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#symptom-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_infection: filterer_infection, id: AppGini.current_symptom__RAND__.value, t: 'student_details', f: 'symptom' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_infection: filterer_infection, s: term, p: page, t: 'student_details', f: 'symptom' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#symptom-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_symptom__RAND__.value, t: 'student_details', f: 'symptom' },
					success: function(resp){
						$j('[name="symptom"]').val(resp.results[0].id);
						$j('[id=symptom-container-readonly__RAND__]').html('<span id="symptom-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=symptoms_view_parent]').hide(); }else{ $j('.btn[id=symptoms_view_parent]').show(); }
