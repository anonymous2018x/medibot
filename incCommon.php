	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'infections' => array('Infections', '', 'resources/table_icons/building.png', 'None'),
			'symptoms' => array('Symptoms', '', 'resources/table_icons/chart_organisation.png', 'None'),
			'online' => array('Online', '', 'resources/table_icons/administrator.png', 'None'),
			...
			'descriptions' => array('Descriptions', '', 'resources/table_icons/clipboard_empty.png', 'None')
		);
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'infections' => "`infections`.`id` as 'id', `infections`.`name` as 'name'",
			'symptoms' => "`symptoms`.`id` as 'id', `symptoms`.`name` as 'name', IF(    CHAR_LENGTH(`infections1`.`name`), CONCAT_WS('',   `infections1`.`name`), '') as 'infection'",
			'class_time_table' => "`class_time_table`.`id` as 'id', `class_time_table`.`day` as 'day', TIME_FORMAT(`class_time_table`.`time_start`, '%r') as 'time_start', TIME_FORMAT(`class_time_table`.`time_end`, '%r') as 'time_end', `class_time_table`.`unit_code` as 'unit_code', `class_time_table`.`venue` as 'venue', IF(    CHAR_LENGTH(`infections1`.`name`), CONCAT_WS('',   `infections1`.`name`), '') as 'infection', IF(    CHAR_LENGTH(`symptoms1`.`name`), CONCAT_WS('',   `symptoms1`.`name`), '') as 'symptom', `class_time_table`.`category` as 'category'",
			'exam_time_table' => "`exam_time_table`.`id` as 'id', if(`exam_time_table`.`date`,date_format(`exam_time_table`.`date`,'%m/%d/%Y'),'') as 'date', TIME_FORMAT(`exam_time_table`.`time_start`, '%r') as 'time_start', TIME_FORMAT(`exam_time_table`.`time_end`, '%r') as 'time_end', `exam_time_table`.`unit_code` as 'unit_code', `exam_time_table`.`venue` as 'venue', IF(    CHAR_LENGTH(`infections1`.`name`), CONCAT_WS('',   `infections1`.`name`), '') as 'infection', IF(    CHAR_LENGTH(`symptoms1`.`name`), CONCAT_WS('',   `symptoms1`.`name`), '') as 'symptom', `exam_time_table`.`category` as 'category'",
			...
			'student_details' => "`student_details`.`id` as 'id', `student_details`.`full_name` as 'full_name', IF(    CHAR_LENGTH(`infections1`.`name`), CONCAT_WS('',   `infections1`.`name`), '') as 'infection', IF(    CHAR_LENGTH(`symptoms1`.`name`), CONCAT_WS('',   `symptoms1`.`name`), '') as 'symptom', `student_details`.`category` as 'category', `student_details`.`reg_no` as 'reg_no'",
			'descriptions' => "`descriptions`.`id` as 'id', `descriptions`.`description` as 'description', IF(    CHAR_LENGTH(`infections1`.`name`), CONCAT_WS('',   `infections1`.`name`), '') as 'infection', IF(    CHAR_LENGTH(`symptoms1`.`name`), CONCAT_WS('',   `symptoms1`.`name`), '') as 'symptom', `descriptions`.`category` as 'category', if(`descriptions`.`date`,date_format(`descriptions`.`date`,'%m/%d/%Y'),'') as 'date'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			...
			'symptoms' => "`symptoms` LEFT JOIN `infections` as infections1 ON `infections1`.`id`=`symptoms`.`infection` ",
			'class_time_table' => "`class_time_table` LEFT JOIN `infections` as infections1 ON `infections1`.`id`=`class_time_table`.`infection` LEFT JOIN `symptoms` as symptoms1 ON `symptoms1`.`id`=`class_time_table`.`symptom` ",
			'exam_time_table' => "`exam_time_table` LEFT JOIN `infections` as infections1 ON `infections1`.`id`=`exam_time_table`.`infection` LEFT JOIN `symptoms` as symptoms1 ON `symptoms1`.`id`=`exam_time_table`.`symptom` ",
			'personal_time_table' => "`personal_time_table` ",
			'student_details' => "`student_details` LEFT JOIN `infections` as infections1 ON `infections1`.`id`=`student_details`.`infection` LEFT JOIN `symptoms` as symptoms1 ON `symptoms1`.`id`=`student_details`.`symptom` ",
			'descriptions' => "`descriptions` LEFT JOIN `infections` as infections1 ON `infections1`.`id`=`descriptions`.`infection` LEFT JOIN `symptoms` as symptoms1 ON `symptoms1`.`id`=`descriptions`.`symptom` "
		);
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('Medibot_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('Medibot_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['Medibot_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['Medibot_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-inverse navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> Medibot</a>
			</div>
